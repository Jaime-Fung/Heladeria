...
export default function MapaUbicacion() {
  ...
  const grouped = locations.reduce((acc, loc) => {
    acc[loc.province] = acc[loc.province] || [];
  ...
  const groupedOptions = orderedProvinces.map((province) => ({
    label: province,
  ...
  const handleChange = (selected) => {
    ...
    if (selected.value === "all") {
      locations.forEach((loc) => {
        L.marker([loc.lat, loc.lon]).addTo(mapInstance).bindPopup(loc.popup);
      });
      mapInstance.setView([9.9376, -84.1043], 9);
    } else {
    ...
    <section className="w-full bg-[#F8EFE3]">
      <div className="mx-auto max-w-6xl px-4 py-10 md:py-14">
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-[1.5fr_1fr] gap-6 md:gap-8">
          <div className="relative rounded-xl overflow-hidden w-full h-[300px] md:h-[450px]">
